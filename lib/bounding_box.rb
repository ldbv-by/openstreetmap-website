class BoundingBox
  ...
  def add_bounds_to(hash, underscore = "")
    hash["min#{underscore}lat"] = format("%<lat>.10f", :lat => min_lat)
    hash["min#{underscore}lon"] = format("%<lon>.10f", :lon => min_lon)
    hash["max#{underscore}lat"] = format("%<lat>.10f", :lat => max_lat)
    hash["max#{underscore}lon"] = format("%<lon>.10f", :lon => max_lon)
    hash
  end
